...
\title{大语言模型人类偏好数据的长度与格式关联分析\\[0.3em]
...
\section{引言}
大语言模型评价常采用成对比较：对同一用户问题展示两个候选回答，由用户选择较优者。该方式能够直接记录实际使用中的相对偏好，Chatbot Arena 等平台据此开展众包评价与模型比较[1]。然而，一个回答被选中并不等于其所有可观察特征都具有同样的质量含义。较长回答可能提供更多必要信息，也可能包含重复解释；标题、列表和粗体可能改善信息组织，也可能仅改变文本外观。对评价数据使用者而言，需要判断选择结果与这些形式特征之间存在多大关联，从而决定哪些配对应进入进一步复核。

已有研究从不同角度讨论偏好信号的构成。Li 等[2]通过细粒度属性分析比较人类与大语言模型的偏好，表明总体选择背后包含多种评价维度。Dubois 等[3]利用回归控制长度因素，改进自动评价指标对长度差异的处理。Zhang 等[4]进一步考察列表、粗体等格式模式，说明格式与偏好学习之间的关系值得独立研究。这些工作为偏好数据审计提供了依据，但人类评价与自动评价器输出是不同的研究对象，对自动评价器的校正效果不能直接转化为人类选择中的因果结论。
...
研究使用 \texttt{lmarena-ai/arena-human-preference-140k} 的本地冻结数据[5]，共 7 个原始分片、135,634 条记录。数据同时包含会话标识、评价次序、双方对话、胜负标签及长度和格式元数据。本文按会话首轮评价建立比较单位，检查角色交替、两侧用户输入轨迹一致性、文本有效性、标签完整性及计数合法性。通过筛选后，对仍存在多条候选记录的会话整体排除，得到每个唯一会话一条记录的分析集。
...
\[x_{Li}=\log\frac{L_{Ai}}{L_{Bi}}.\qquad\text{(2.1)}\]
...
\[x_{fi}=1000\left(\frac{C_{fAi}}{L_{Ai}}-\frac{C_{fBi}}{L_{Bi}}\right).\qquad\text{(2.2)}\]
...
\caption{表1 主要变量及统计含义}\label{tab:vars}
\begin{tabular}{lll}
\toprule
变量 & 定义与来源 & 解释边界\\
\midrule
...
\section{成对比较模型与推断方法}
\subsection{描述性配对比较}
以胜者减败者的差值描述方向，报告非零差配对中正差比例、零差数及秩二列相关。符号检验及格式存在性的精确McNemar检验沿用冻结分析；全量、英语、单轮及任务组合的比较采用同一Holm家族校正[6]。这些描述检验以配对独立为工作假设，仅作背景，不替代下文的聚类回归推断。补充Wilcoxon结果不作无条件的中位数检验解释。
...
\[\log\frac{p_i}{1-p_i}=\alpha+\beta_Lz_{Li}+\beta_Hz_{Hi}
 ...
 +\theta_{m_{Ai}}-\theta_{m_{Bi}}.\qquad\text{(3.1)}\]
模型身份参数差沿用Bradley--Terry型结构[7,8]，通过指定参考模型识别。本文称其为含模型身份对比项的成对比较Logistic模型，区别于匹配病例的条件Logistic回归。共有任务变量及截距允许A侧优势随背景变化，不将模型宣称为严格交换对称的纯排名模型。参数通过最大化Bernoulli对数似然估计：
\[\ell(\bm\eta)=\sum_i\{Y_i\bm v_i^\top\bm\eta-\log[1+\exp(\bm v_i^\top\bm\eta)]\}.\qquad\text{(3.2)}\]
...
设组$g$的得分和为$\bm s_g=\sum_{i\in g}\bm v_i(Y_i-\widehat p_i)$，$B=\sum_i\widehat p_i(1-\widehat p_i)\bm v_i\bm v_i^\top$，聚类协方差为[9]
\[\widehat V_{\mathrm{cl}}=\frac{G}{G-1}\frac{n-1}{n-K}
 B^{-1}\left(\sum_{g=1}^G\bm s_g\bm s_g^\top\right)B^{-1},\qquad\text{(3.3)}\]
...
长度项使用四节点限制性三次样条[10]，节点规则在拟合前固定为全量长度指标的5\%、35\%、65\%、95\%分位数；保留线性项并增加两个非线性基函数。对两个非线性系数联合检验，聚类方案采用$F_{2,G-1}$近似。AIC只作为条件独立似然下的辅助描述。

预测曲线将有方向长度比$r=L_A/L_B$固定在给定值，对观察样本其余协变量分布平均：
\[\widehat P(r)=\frac1n\sum_i\operatorname{logit}^{-1}
 \{\widehat\eta_i+\widehat f(z_L(r))-\widehat f(z_{Li})\}.\qquad\text{(3.4)}\]
...
\[\operatorname{logit}\Pr(D_i=1)=\alpha_D+\delta_Lw_{Li}
 ...
 +\phi_{m_{Ai}}+\phi_{m_{Bi}}+\lambda_{t_i},\qquad\text{(3.5)}\]
...
\subsection{审计分层}
定义无方向长度比
\[R_i=\frac{\max(L_{Ai},L_{Bi})}{\min(L_{Ai},L_{Bi})},\qquad\text{(3.6)}\]
...
表2采用首提示词聚类推断。全量长度OR为1.6196；其单位为长度对数比增加全量标准差1.0224，相当于原长度比乘以约2.780，不是增加一个token。该OR是线性设定的平均斜率摘要，后文的非线性检验表明不能将其视为所有长度区间共同适用的恒定关联。

\begin{table}[htbp]\centering\small\caption{表2 统一标准化下的条件选择关联}\label{tab:reg}
...
\begin{figure}[htbp]\centering\includegraphics[width=15cm]{assets/adjusted_associations.png}
\caption{图1 三个重叠样本的条件关联。统一标准化尺度，首提示词聚类逐项区间；左右面板横轴范围不同。}\label{fig:reg}\end{figure}
...
\begin{figure}[htbp]\centering\includegraphics[width=15cm]{assets/length_curve.png}
\caption{图2 长度与选择的标准化预测关联。阴影为首提示词聚类Delta法95\%逐点区间，固定观察到的格式密度与其他协变量，横轴为A/B有方向长度比。}\label{fig:curve}\end{figure}

图2显示线性与样条在不同长度范围的拟合差异。曲线展示0.25至4的长度比，该区间位于观察长度比的1\%至99\%分位范围内；但边际范围内不保证全部协变量组合都有共同支持。非线性显著不自动确定最优复核阈值，也不证明某一长度比例是因果拐点。
...
平局和双方差合计29,195条，占保留样本约26.99\%。表3考察是否产生明确胜负，单位为全部保留样本中绝对差指标的一个标准差。它与表2使用不同结果变量和尺度，不能横向比较OR大小或把状态模型称为选择偏差校正。

\begin{table}[htbp]\centering\small\caption{表3 形成明确胜负的关联}\label{tab:decisive}
...
\[z_{ki}=\alpha_k+a_kz_{Li}+\bm\gamma_k^\top\bm X_i
 +\theta_{k,m_{Ai}}-\theta_{k,m_{Bi}}+\varepsilon_{ki}.\qquad\text{(5.1)}\]
选择方程使用式(3.1)，格式系数记为$b_k$。方向为事后探索设定，无时间先后或干预识别；不使用由同一样本胜率构造的能力代理，也不因列表不显著而删除其路径。
...
\begin{table}[htbp]\centering\small\caption{表4 探索性路径乘积}\label{tab:paths}
...
按式(3.6)划分三层，保留明确胜负、平局和双方差。表5展示真实样本；较长者获胜的分母仅为本层不等长且明确胜负配对，分别为15,777、27,017、35,978。

\begin{table}[htbp]\centering\small\caption{表5 真实长度分层及评价构成}\label{tab:audit}
...
\begin{figure}[htbp]\centering\includegraphics[width=15cm]{assets/audit_strata.png}
\caption{图3 各长度层中较长回答获胜比例。精确二项区间仅为配对独立假设下的描述区间，未作聚类修正。}\label{fig:audit}\end{figure}
...
\[\widehat A=\sum_h W_h\widehat p_h,\qquad
 \widehat p_h=b_h^{-1}\sum_{j=1}^{b_h}a_{hj}.\qquad\text{(6.1)}\]
在各层独立抽样条件下，其设计方差为[11]
\[\operatorname{Var}(\widehat A)=\sum_hW_h^2
 \left(1-\frac{b_h}{N_h}\right)\frac{S_h^2}{b_h},
 \qquad S_h^2=\frac{1}{N_h-1}\sum_{j=1}^{N_h}(a_{hj}-p_h)^2.\qquad\text{(6.2)}\]
...
\[b_h=B\frac{N_hS_h}{\sum_kN_kS_k}.\qquad\text{(6.3)}\]
