...
> 2026-09-12 修订：当前权威入口为 `Codes/reproduce.py`，参见 [可复现流程](REPRODUCIBILITY.md)。下面 v1.6 内容及旧论文、表格属于历史档案，不能作为最新统计结论。旧因果、IPW、匹配与 SEM 推断已撤回；新流程只报告经校正的观察性关联。
...
| C16 | length_test | Wilcoxon 长度偏好检验 + Bonferroni + Bootstrap CI + Cohen's d | R13 + T03 + P06 ✅ |
| C17 | format_test | 格式偏好检验（Wilcoxon + 密度辅助 + 卡方）| R14 + T04 + P07 ✅ |
| C18 | pure_effect | 净效应嵌套逻辑回归（长度 M0→M3；格式 F0→F3；C13 子集）| R16 + T05-T06 + P08-P09 ✅ |
| C19 | length_effect_robust | 调整 Logit + 稳定化 IPW + bootstrap CI | R17 + T07 + P10 ✅ |
| C20 | format_effect_robust | 格式效应稳健性分析（调整 Logit + 稳定化 IPW + bootstrap CI） | R18 + T08 + P11 ✅ |
| C21 | enhanced_matching_diagnostics | PSM 匹配诊断 + SMD 平衡性 + 配对 Wilcoxon | R19 + T09-T10 + P12 ✅ |
| C22 | sem_analysis | 结构方程模型（主模型/扩展模型 + bootstrap CI + 路径图）| R20 + T11-T15 + P13-P14 ✅ |
