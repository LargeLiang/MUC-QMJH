...
当前版本的导读不再只是解释“这些方法为什么存在”，还要和已经完成的论文终稿保持一致。阅读时建议优先记住下面四点：

- 研究结论的权威口径以 `END/paper.md` 和对应 `Reports` 为准，而不是早期草稿或单张图表。
- 长度偏好是全项目最稳健的经验发现，但在机制层面要谨慎表述，不能直接说“长度本身的直接因果路径已经被完全证明”。
- 标题密度和粗体密度是更稳定的独立格式信号；列表密度只保留为敏感性变量，不作为正式主结论。
- 第四、第五阶段的导读现在按 C18-C22 的稳健性链条和 C23 的 SEM 机制分析来理解，不再沿用旧版脚本编号。

---

## 4. 配套文件

- `statistical_logic_chain.md`：把 M01-M23 按课题真实推进顺序压缩成一张逻辑链条图，适合先建立“方法为什么按这个顺序出现”的全局地图。
...
- `method_list.md`：把 C14-C23 真正进入结果解释的统计方法按首次出现顺序整理为 M01-M23，适合和 Phase 3-5 搭配阅读。
